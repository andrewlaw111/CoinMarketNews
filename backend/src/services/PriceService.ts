...
export default class CoinService {
    ...
    public getBySearch(token: string, searchInput: string, fiat: string = "USD", crypto: string = "BTC") {
        return new Promise((resolve, reject) => {
            // Make a query to the database if the list has not been updated for 5 minutes
            if (Date.now() - this.lastUpdated < 300000) {
                const findcoins = this.selectCurrency(fiat, crypto).filter((coin) =>
                    RegExp(searchInput.toLowerCase()).test(coin.name.toLowerCase()) ||
                    RegExp(searchInput.toLowerCase()).test(coin.symbol.toLowerCase()));
                resolve(findcoins);
            } else {
                this.updatePriceList()
                    .then(() => {
        ...
        return knex("price")
            .then((prices: any[]) => {
                for (const price of prices) {
                    ...
                    price.price = (price.currency_id === 6) ?
                        parseFloat(price.price).toFixed(8) : parseFloat(price.price);
    ...
    private selectCurrency(fiat: string = "USD", crypto: string = "BTC", start: string = "0", limit: string = "100") {
        ...
        mapSymbolID.BTC = 6;
        mapSymbolID.ETH = 7;
        const startInt = parseInt(start, undefined);
        const nb = startInt + parseInt(limit, undefined);
        const priceList = this.priceList.slice(startInt, nb);
        for (const coin of priceList) {
            if (coin.coinmarketcap_id in this.prices && mapSymbolID[fiat] in this.prices[coin.coinmarketcap_id]) {
                coin.price_fiat = this.prices[coin.coinmarketcap_id][mapSymbolID[fiat]];
            }
            if (coin.coinmarketcap_id in this.prices && mapSymbolID[crypto] in this.prices[coin.coinmarketcap_id]) {
                coin.price_crypto = this.prices[coin.coinmarketcap_id][mapSymbolID[crypto]];
            }
        }
        return priceList;
    }
